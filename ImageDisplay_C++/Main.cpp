...
// Global Variables:
MyImage			inImage, outImage,interImage;				// image objects
                     ...
                     int       nCmdShow)
{
	...
	sscanf(lpCmdLine,"%s %f", &ImagePath,&n);
	inImage.setWidth(w);
	inImage.setHeight(h);
	inImage.setN(n);
	...
	if(inImage.getN()==-1)
					{
						...
						float coeff;
						for(coeff=4096;coeff<=262144;coeff=coeff+4096)
						{
							
						inImage.setN(coeff);
						outImage.setN(coeff);

						inImage.Modify(); //left  image dct
						outImage.Dwt();   //right image dwt
